...
class _HomeShellV4State extends State<HomeShellV4> {
  ...
  void _select(Note note) => setState(() {
    if (!selectedIds.add(note.id)) selectedIds.remove(note.id);
  });
  Future<void> _bulk(String action) async {
    final chosen = _active.where((n) => selectedIds.contains(n.id)).toList();
    String? destination;
    if (action == 'move') {
      var value = '';
      destination = await showDialog<String>(
        ...
        builder: (ctx) => AlertDialog(
          title: const Text('Mover notas para pasta'),
          content: TextField(
            autofocus: true,
      ...
      savedFolders.add(destination);
      await _saveCollections();
    }
    for (final note in chosen) {
      if (action == 'move') note.folder = destination!;
      if (action == 'favorite') note.favorite = true;
      if (action == 'archive') note.archived = true;
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${chosen.length} notas atualizadas')),
      );
    }
  }
  ...
  Future<void> _loadCollections() async {
    ...
    if (!mounted) return;
    setState(() {
      sortOrder = prefs.getInt('notoSortOrder') ?? 0;
  ...
  List<Note> get _visible {
    ...
    list.sort((a, b) {
      ...
      if (sortOrder == 1) return a.updatedAt.compareTo(b.updatedAt);
      if (sortOrder == 2)
  ...
  Future<void> _openNote(Note note, {bool isNew = false}) async {
    await Navigator.push<bool>(
      ...
      MaterialPageRoute(
        builder: (_) =>
            EditorPageV2(store: widget.store, note: note, isNew: isNew),
      ),
    );
  }
            ...
            children: [
              ...
              Expanded(
                child: widget.store.templates.isEmpty
                    ? const Center(
                        child: Text(
                          'Salva uma nota como modelo pelo menu do editor.',
                        ),
                      )
                    : ListView.builder(
                        ...
                        itemBuilder: (_, index) {
                          ...
                          return ListTile(
                            leading: Text(
                              template.emoji.isEmpty ? '▣' : template.emoji,
                              style: const TextStyle(fontSize: 20),
                            ),
                            title: Text(template.name),
                            subtitle: Text(
                              template.title.trim().isEmpty
                                  ? 'Sem título pré-definido'
                                  : template.title,
                              maxLines: 1,
                            ),
                            trailing: IconButton(
          ...
          child: ListView(
            ...
            children: [
              ...
              if (widget.store.templates.isNotEmpty) ...[
                ...
                ...widget.store.templates
                    .take(8)
                    .map(
                      (template) => _CreateRow(
                        title:
                            '${template.emoji.isEmpty ? '' : '${template.emoji} '}${template.name}',
                        subtitle: template.title.trim().isEmpty
            ...
            children: [
              ...
              DropdownButtonFormField<int>(
                ...
                items: const [
                  DropdownMenuItem(
                    value: 0,
                    child: Text('Qualquer prioridade'),
                  ),
                ...
                onChanged: (value) =>
                    setModalState(() => draftPriority = value ?? 0),
              ),
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
                ...
                value: draftReminder,
                onChanged: (value) =>
                    setModalState(() => draftReminder = value),
              ),
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
                ...
                onChanged: (value) =>
                    setModalState(() => draftChecklist = value),
              ),
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
  ...
  void _openCalendar() => Navigator.push(
    ...
    MaterialPageRoute(
      builder: (_) =>
          CalendarPage(store: widget.store, onOpen: (note) => _openNote(note)),
    ),
  );
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (_, __) {
      ...
      final archived = widget.store.notes
          ...
          .length;
      final trash = widget.store.notes
                  ...
                  children: [
                    ...
                    PopupMenuButton<int>(
                      ...
                      onSelected: (value) async {
                        ...
                        final prefs = await SharedPreferences.getInstance();
                        await prefs.setInt('notoSortOrder', value);
                      },
                      itemBuilder: (_) => const [
                        PopupMenuItem(value: 0, child: Text('Mais recentes')),
                        PopupMenuItem(value: 1, child: Text('Mais antigas')),
                        PopupMenuItem(value: 2, child: Text('Título A–Z')),
                        PopupMenuItem(value: 3, child: Text('Título Z–A')),
                      ],
                    ),
                    if (folder != null)
                      Flexible(
            ...
            : SafeArea(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(child: Text('${selectedIds.length} selecionadas')),
                    IconButton(
                      tooltip: 'Selecionar todas visíveis',
                      onPressed: () => setState(
                        () => selectedIds.addAll(_visible.map((n) => n.id)),
                      ),
                    ...
                    PopupMenuButton<String>(
                      ...
                      itemBuilder: (_) => const [
                        ...
                        PopupMenuItem(
                          value: 'favorite',
                          child: Text('Favoritar selecionadas'),
                        ),
                        PopupMenuItem(
                          value: 'archive',
                          child: Text('Arquivar selecionadas'),
                        ),
                      ],
                    ),
                  ],
...
class _TodayLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (snapshot.pendingTasks > 0)
      parts.add('${snapshot.pendingTasks} pendentes');
    if (snapshot.remindersToday > 0)
      parts.add('${snapshot.remindersToday} lembretes');
    if (snapshot.overdueReminders > 0)
      parts.add('${snapshot.overdueReminders} atrasados');
...
class _NoteCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final noteColor = NotoAppearance
        .noteColors[NotoAppearance.safeNoteColorIndex(note.color)];
                      ...
                      children: [
                        ...
                        Wrap(
                          ...
                          children: [
                            ...
                            if (note.dueAt != null)
                              Text(
                                'até ${DateFormat('dd/MM').format(note.dueAt!)}',
                                style: TextStyle(
                                  color: metadataColor,
                                  fontSize: 9.5,
                                  fontWeight: FontWeight.w700,
